...
export default function RoyAvatarFX({
  ...
  const [mouthScale, setMouthScale] = useState(1.2);
  ...
  useEffect(() => {
    ...
    const data = alignment as any;
    const times = data?.characterStartTimesSeconds ?? [];
    ...
    if (data?.characters && data?.characterStartTimesSeconds) {
      setMouthScale(1);
      mouthTimer.current = setInterval(() => {
        mouthIndex.current++;
        setMouthFrame(mouthIndex.current);
        setMouthScale(0.8 + (mouthIndex.current % 3) * 0.3);
        if (mouthIndex.current >= times.length) mouthIndex.current = 0;
      }, 50);
      const duration = data.characterStartTimesSeconds.at(-1) || 0;
      if (duration > 0) {
        setTimeout(() => setMouthScale(1.2), duration * 1000);
          if (mouthTimer.current) {
            clearInterval(mouthTimer.current);
            mouthTimer.current = null;
          }
  ...
  return (
    ...
    >
      <div
        className="absolute w-[22rem] h-80 rounded-full"
        style={{
          background:
      ...
      <motion.div
        className={`absolute w-[22rem] h-80 rounded-full border ${ringClass}`}
        animate={{
          scale: [1, 1.04, 1],
          opacity: [0.6, 1, 0.6],
        }}
        transition={{
          duration: state === "speaking" ? 1.8 : 3,
          repeat: Infinity,
          ease: "easeInOut",
        }}
      />
      ...
      {state === "speaking" && (
        <motion.img
          src="/avatar/roy-mouth-new-layer.png"
          alt="Roy talking mouth"
          className="absolute top-[31%] left-1/2 -translate-x-1/2 w-64 pointer-events-none z-20"
          animate={{
            scaleY: [
              0.8,
              0.75 + audioLevel * 10 * mouthScale + (mouthFrame % 3) * 0.2,
              0.9,
              0.85 + audioLevel * 8 * mouthScale + (mouthFrame % 2) * 0.15,
              0.8,
            ],
            scaleX: [1, 1 + audioLevel, 1],
          }}
          transition={{
            duration: 0.16,
            repeat: Infinity,
