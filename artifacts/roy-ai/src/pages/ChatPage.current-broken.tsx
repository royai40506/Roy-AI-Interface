...
function playBase64Audio(base64: string) {
  try {
    ...
    audio.volume = 1;
    audio.onplay = () => console.log("ROY AUDIO PLAY START");
...
function speakAssistant(_text: string, _language: string) {
  console.log("Browser TTS disabled - ElevenLabs only");
}
    ...
    async (content: string, attachedFile?: string) => {
      ...
      try {
        await streamChat({
          ...
          onDone: (full) => {
                console.log("ROY ONDONE HIT", full.length);
                setTtsDebug(`ONDONE HIT | ${full.length}`);
            setIsTyping(false);
            setStreamingText('');
            if (full.trim()) {
              ...
              generateVoice({
                ...
                setTtsDebug(
                  audio
                    ? `ELEVEN AUDIO OK | length:${audio.length}`
                    : "ELEVEN AUDIO EMPTY"
                );
              }).finally(() => {
                setOrbState("idle");
